...
def serve_select(
    ...
    async def get(
        request: Request,
        body: filter_model | None = Body(None),  # type: ignore
        ...
        """
        # Use body data if provided (for POST requests with JSON),
        # otherwise use filters from Depends() (for GET requests with query params)
        actual_filters = body if body is not None else filters
        ...
        select_query_str = (
            select_query(actual_filters) if callable(select_query) else select_query
        )
        parsed_select_params = get_parsed_select_params(
            actual_filters, limit, offset, select_param, param
        )
        # Build initial count parameters
        count_query_str = count_query(actual_filters) if callable(count_query) else count_query
        parsed_count_params = get_parsed_count_params(actual_filters, count_param, param)

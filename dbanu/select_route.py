...
def serve_union(
    ...
    # Create the route with dependencies
    @app.get(path, response_model=list[data_model], dependencies=wrapped_dependencies)  # type: ignore
    def get(
        filters: filter_model = Depends(),  # type: ignore
        limit: int = 100,
        offset: int = 0,
        ...
        for source in sources:
            ...
            query_processor = _create_query_processor(query_engine, SelectResponseModel)
            handler = _create_middleware_chain(middlewares)
            result.append(handler(initial_context))
        return result
